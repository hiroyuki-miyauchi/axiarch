...
>
> 1.  These documents (`axiarch-rules/universal/*.md`) are the **Supreme Law** of this project's development, operations, and business.
...
*   **Language Selection**:
    ...
    *   **Rule Application**: The AI strictly adheres to the language setting defined in `AGENTS.md` for all communication and thought processes. Please delete the unused language directories (in `axiarch-rules/universal/` and `axiarch-rules/blueprint/`) upon project initialization.
...
*   **Universal Rules (Immutable)**: `axiarch-rules/universal/` is the DNA of the Axiarch framework. No unauthorized changes are allowed.
*   **Blueprint Rules (Mutable)**: Project-specific circumstances are managed in `axiarch-rules/blueprint/`.
