# Engineering

> This folder is the destination for project-specific rules
> crystallized from `core/010_project_lessons_log.md` for the **Engineering domain**.

> [!NOTE]
> **The correct initial state is that no domain rule files exist here other than this README.**
> Domain rule files are generated when lessons accumulated through real-world practice reach the threshold (3 entries), following `CRYSTALLIZATION_PROTOCOL.md`.
> Pre-populating seed files is prohibited as it violates the YAGNI principle.

## Corresponding Universal Rules

| File | Description |
|:-----|:------------|
| `universal/engineering/000_engineering_standards.md` | Engineering Standards |
| `universal/engineering/100_api_integration.md` | API & Integration Standards |
| `universal/engineering/200_supabase_architecture.md` | Supabase Architecture |
| `universal/engineering/300_web_frontend.md` | Web Frontend Standards |
| `universal/engineering/310_headless_cms.md` | Headless CMS Standards |
| `universal/engineering/400_mobile_flutter.md` | Mobile (Flutter) Standards |
| `universal/engineering/410_native_platforms.md` | Native Platform Standards |
| `universal/engineering/500_firebase_gcp.md` | Firebase & GCP Standards |
| `universal/engineering/510_aws_cloud.md` | AWS Cloud Standards |

## Operating Guide

- Follow `axiarch-rules/{lang}/CRYSTALLIZATION_PROTOCOL.md` for the lesson crystallization process
- File numbering uses 3-digit numbers with any available value from `000` to `999`. Incrementing by 10 is a recommended convention, not a reserved band; `000` may be used when appropriate.
- AI must not autonomously create new domain folders. Prefer existing folders; if a new domain cannot be classified, propose an extension and create it only after user approval.
