...
- **Contextual Numbering**: (1) **recommended style** = gaps of about 5–10 near related topics, leaving room; (2) **in practice** = if no gap is available, use an interstitial number (e.g., `011`, `015`). Either is valid as long as it is an unused number from `000`–`999` in the target folder. Prioritize avoiding exhaustion (i.e., simply use an available number) over strictly enforcing fixed gaps.
