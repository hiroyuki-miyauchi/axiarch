...
> **`{NNN}` の決定方法 (Contextual Numbering)**: 昇華時に対象フォルダ内の既存ファイルをAIが実際に確認して採番する。指針は単純で、(1) **推奨スタイル**＝関連トピックの近隣に、拡張余地を残す約5〜10刻み（例: `010`, `020`）、(2) **実際**＝空きが無ければ間の番号（例: `011`, `015`）を使う。いずれも対象フォルダ内の `000`〜`999` の未使用番号であれば有効。固定刻みの厳密化より、枯渇回避（=空き番号を素直に使うこと）を優先する。これは `universal/core/100_governance.md` §8.1 の Sparse Numbering スタイル推奨と一致する。
