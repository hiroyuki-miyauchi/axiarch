...
パス基準: 本書の `core/` 等のカテゴリ略記は `axiarch-rules/ja/blueprint/`、`universal/` と `blueprint/` は `axiarch-rules/ja/` を起点とする。実記録には解決したパスを残し、Markdownリンクは記録ファイルからの相対パスで作る。番号付きのファイル名例は実在・必須採用を意味しない。
...
AIがあるドメインフォルダ（例: `engineering/`）をロードするとき、そこにはルールだけでなく過去の教訓も存在するー関連する知見を同じ探索範囲で見つけやすくする。配置だけで自動ロード済みにはならない。
...
> **`{NNN}` の決定方法 (Contextual Numbering)**: 昇華時に対象フォルダ内の既存ファイルをAIが実際に確認して採番する。指針は単純で、(1) **推奨スタイル**＝関連トピックの近隣に、拡張余地を残す約5〜10刻み（例: `010`, `020`）、(2) **実際**＝空きが無ければ間の番号（例: `011`, `015`）を使う。いずれも対象フォルダ内の `000`〜`999` の未使用番号であれば有効。固定刻みの厳密化より、枯渇回避（=空き番号を素直に使うこと）を優先する。これは `axiarch-rules/ja/universal/core/100_governance.md` §8.1 の Sparse Numbering スタイル推奨と一致する。
...
**判定基準:**
- Universalに同趣旨のルールが存在 → 新しい固有事情がなければ記録不要（重複回避）。新規記録の有無にかかわらずStep 5へ進む。
...
Step 1 で特定した **Blueprint フォルダ内** に同ドメインのルールファイル（`{NNN}_{topic}.md`）が既に存在する場合、そのファイルを直接読み、必要な差分を追記する。その後Step 5・Step 6まで確認する。
...
> [!CAUTION]
> **🚨 「追記 = 完了」は誤認 — 教訓記録を閉じる前に Step 5 を実行せよ**
>
> 過去の実装で「`core/010` に追記したから結晶化完了」と誤認し、3件以上溜まったドメインを Blueprint へ昇華せず放置するケースが頻発した。**Step 4 (ACCUMULATE) は完了ではない**。教訓を作成・更新するタスクでは、AIは完了前に Step 5 (THRESHOLD CHECK) を必ず実行し、**(a) 3件以上のドメイン、または (b) 180 日以上前の lesson があれば Blueprint 専用ファイルへの昇華まで完了させてから** タスク完了を宣言すること。
>
...
**採番ルール:**
- **採番範囲**: 各ドメイン内で `000`〜`999` を独立して採番する。特定ドメイン専用の予約帯域は存在しない。
- **文脈的採番 (Contextual Numbering)**: (1) **推奨スタイル**＝関連トピックの近隣に約5〜10刻みで拡張余地を残す。(2) **実際**＝空きが無ければ間の番号（例: `011`, `015`）を使う。いずれも対象フォルダ内の `000`〜`999` の未使用番号であれば有効。固定刻みの厳密化より枯渇回避（=空き番号を素直に使うこと）を優先する。
...
件数・経過日の検査はインストール済みの両言語を対象とするが、採用先の教訓を機械的に翻訳・複製しない。引用・コードブロック内のテンプレートは件数に含めず、実記録には日付、Domain、Target Folderを記す。期限回避のために元の日付を更新しない。新フォルダの承認待ち等で昇華できない場合は保留理由と再開条件を記録し、昇華済みと報告しない。H0の読取のみには教訓更新や全ログ整理を要求しない。

### Step 6: UPDATE INDEX (インデックス更新)

`axiarch-rules/{lang}/blueprint/core/010_project_lessons_log.md` の分離済み一覧と `axiarch-rules/{lang}/blueprint/INDEX.md` の対象カテゴリを更新し、作成・追記先の目次と逆引き索引も実内容へ同期する。言語を対として管理する規則は両言語で対応させ、利用先の固有状態を別言語へ機械コピーして正本を増やさない。
...
| 必須項目 | 説明 |
|:-------------------|:-------------------|
| `Domain` | ドメイン名（初期分類または承認済み追加カテゴリ） |
...
|:--|:-----------|:-----|:--------------|
| 1 | **タイトル行** `# {NNN}. {名称}` | ファイル冒頭のH1見出し。番号 + 名称 | `# 200. 言語プロトコル`（universal/core）、`# 000. エンジニアリング基準`（universal/engineering） |
...
**判定基準**: 「このファイルを Universal の `axiarch-rules/{lang}/universal/engineering/000_engineering_standards.md` や `axiarch-rules/{lang}/universal/core/200_language_protocol.md` と並べたとき、構造的に違和感がないか？」→ 違和感があれば揃える。
