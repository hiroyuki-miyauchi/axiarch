...
> [!TIP]
> このファイルは、`axiarch-rules/{lang}/blueprint/` の対応ドメインフォルダにコピーして使用します。ファイル名はフォルダ内 000〜999 の空き番号に変更してください（例: `security/010_security_policy.md`）。番号は衝突回避のみが目的で、予約された番号帯域はなく、`000` の使用も必須ではありません。
...
| フォルダ | 使用可能番号 | 用途例 |
|:--------|:------------|:------|
| `core/` | 000〜999 の空き番号（既存使用済みを除く） | 教訓ログ（`010_`）、結晶化ルール、プロジェクト固有設定 |
| `security/` | 000〜999 の空き番号 | セキュリティポリシー、セキュリティ機能仕様 |
| `engineering/` | 000〜999 の空き番号 | システム固有ルール、エンジニアリング機能仕様 |
| `design/` | 000〜999 の空き番号 | デザインシステム上書き、デザイン機能仕様 |
| `quality/` | 000〜999 の空き番号 | テスト戦略固有、QA機能仕様 |
| `operations/` | 000〜999 の空き番号 | SRE・CI/CD固有、運用機能仕様 |
| `product/` | 000〜999 の空き番号 | ビジネスルール、機能仕様（最も一般的な配置先） |
| `ai/` | 000〜999 の空き番号 | AI戦略固有、AI機能仕様 |

> **採番ルール**: 各フォルダ内で `010_`, `020_`, `030_`... の**10単位間隔**で採番するのが推奨例です（将来の挿入余地を確保）。ただし 10単位間隔は推奨であって固定帯域ではなく、`000` を含む空き番号を自由に使用できます。フォルダを越えた全体連番は使いません。
