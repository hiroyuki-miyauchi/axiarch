...
7. **Brownout Testing（ブラウンアウト試験）**: 廃止前に時限的無効化を**最低1回**実施し、残存利用者を顕在化させる（SHOULD）
   - 短時間（数分〜数時間）だけ `503` / `410` を返す等の計画的なブラウンアウトで、監視だけでは見えない残存クライアントを実トラフィックで検出する
   - 先行例: GitHubのSHA-1署名廃止ブラウンアウト（2026）、GitHub API認証方式廃止ブラウンアウト（2021）
   - ブラウンアウト中も機械可読シグナル（`Deprecation` / `Sunset` ヘッダー、RFC 8594）を併用し、クライアント側が原因を機械的に特定できるようにする
   - 内部APIにも同一手順を適用する。「社内利用だから即時停止してよい」という運用を禁止する
...
- APIドキュメントは原則としてデフォルト言語版で作成し、必要に応じてセカンダリ言語版を併設する
...
- API経由のデータ転送においてデータレジデンシー要件（例: GDPR、日本のPrivacy Laws、および同様の法域のプライバシー法等）を遵守する
