...
*   **Action**:
    ...
    4.  **llms.txt / AXIARCH.md / thin adapters**: プロジェクトルートでは `llms.txt` と正本である `AXIARCH.md` にAIエージェント向けの概要、アーキテクチャ、制約を集約してください。`AGENTS.md` やツール固有ファイルは `AXIARCH.md` への薄いポインターに留め、ルール本文を重複させてはいけません。
