...
> 改定日: 2026-09-11（言語の適用範囲・実行契約との整合）

> [!IMPORTANT]
> **Primary Directive（主要方針）**
> 読み手と出力先に合う言語で、判断と作業結果を伝える。
> - 基本原則: AIの応答言語の優先順位は、プロジェクトルートの `AXIARCH.md` §1を正本とする。ユーザーの最新の明示指定を優先し、指定がなければ `Project Native Language` を既定とする。英語を選んだ利用先では英語が正しい出力になる。
> - 適用範囲: ユーザーに提示する計画・判断理由の要約・報告・質問と、対象読者に向けたUI・文書。AI内部の非公開の思考を開示・検査する義務にはしない。
> - 技術上の表記: コード識別子、API、ログ、パッケージ名、引用等の必要な原表記を保持する。異なる言語であることだけで違反とせず、出力先の設定と文脈に照らして判断する。

Universalは共通の言語選択・品質の原則、Blueprintは既定言語・対象ロケール・用語集・レビュー基準という可変の設定を担う。任意のプロンプト層は必要な規則のロードと実行を補助する。以下の必須条項は対象機能・読者が存在する範囲へ適用し、推奨・例示の技術や定期運用は、明示的に必須化されていない限り任意である。列挙されたSDK、通知、RAG、多言語製品を全導入先へ追加する義務はない。

製品の表示言語は対応ロケールと利用者の選択に従う。「英語残存」等の例は出力先が英語以外の場合の問題であり、英語自体を禁止する意味ではない。Axiarch配布の自動解決は現在ja/enを対象とする。設定を解決できたことと、内容の正しさ・翻訳品質・ロード完了は別である。検査とレビューで最低品質の底上げを図り、誤りの完全な排除は保証しない。
...
- **Rule 74.3**: 既定言語の設定値はプロジェクト固有の変更として、既存の承認範囲で更新し影響を記録する。Universal条文を変更する場合だけ [core/100_governance.md](./100_governance.md) Part IVの改定手続きを適用する。

### 1.2. 言語使用の三層モデル

全テキストを以下の三層に分類し、各層で言語ポリシーを適用する：

ここでのL1–L3は言語の用途分類であり、自律距離D・成熟度M・実行ハーネスHとは別軸である。

| 層 | 対象 | 言語 | 根拠 |
|---|---|---|---|
| **L1: ユーザー層** | UI・通知・メール・エラーメッセージ・ヘルプ | 選択された表示言語（既定はプロジェクト設定言語） | 翻訳負荷の軽減 |
| **L2: 開発者層** | 設計書・PR・タスク・コミット要約・コードコメント | 適用する応答言語。コード・コード内コメントはPart IIに従う | チーム内意思疎通の正確性 |
| **L3: システム層** | ログ・メトリクス・監査証跡・外部API | システムの契約とPart IX・XIIに従う | 国際相互運用性・ツール互換 |
...
│  (UI, 通知, メール, エラー)           │
├─────────────────────────────────────┤
│  L2: 開発者層                        │ → 適用する応答言語。コードはPart II
│  (設計書, PR, タスク, コメント)       │
├─────────────────────────────────────┤
│  L3: システム層                      │ → システムの契約とPart IX・XII
│  (ログ, メトリクス, 監査, 外部API)    │
└─────────────────────────────────────┘
```
...
- **Rule 74.4**: 対象読者に不要な翻訳作業を負わせる表現を、出力先の言語と文脈に照らして見直す。
- **Rule 74.5**: システム生成の既定メッセージが選択された表示言語や用途に合わなければ修正する。英語が選択されている場合、英語であること自体は不具合ではない。
- **Rule 74.6**: 翻訳の質も対象。直訳調の不自然な表現（`"それは成功しました"`, `"無効な値です"`）も禁止。自然なプロフェッショナル表現（`"保存しました"`, `"正しい形式で入力してください"`）を義務付ける

### 1.4. 言語ネゴシエーション戦略

- **Rule 74.7**: 製品UIの言語選択では、利用者の現在の明示選択を優先する。以下は対応ロケール内での解決例であり、AI応答の優先順位は `AXIARCH.md` §1に従う。位置による推定は任意で、利用目的・許可範囲を確認し、言語選択だけのために精密位置情報を収集しない：

```
1. 利用者の現在の明示選択
2. DB保存のユーザー言語設定
3. セッション/Cookie に保存された言語選択
4. HTTP `Accept-Language` ヘッダー
5. `navigator.language` / `navigator.languages`
6. 許可・採用された場合だけ地域推定
7. プロジェクトデフォルト言語
```

- **Rule 74.8**: 製品が単一ロケールだけを提供する場合、言語ネゴシエーションは不要。既定言語が一つでも複数ロケールを提供できるため、両者を同一視しない。
- **Rule 74.9**: 言語切替時のUI状態（フォーム入力値・スクロール位置・モーダル状態）を確実に保持する
- **Rule 74.9A**: Service Workerによる言語別プリフェッチ・キャッシュは、導入済み機能と効果を評価した上で選べる任意の最適化とする。
- **Rule 74.9B**: 日時処理では言語・タイムゾーン・暦を別の設定として扱う。Temporalを採用する場合、`Temporal.Now.zonedDateTimeISO(timeZone)` はISO暦を返し、別の暦が必要なら結果へ `withCalendar()` を適用する。言語だけで地域やタイムゾーンを断定しない。APIの対応状況は実行環境で確認する（[公式仕様解説](https://tc39.es/proposal-temporal/docs/now.html)）。

### 1.5. 言語ポリシーガバナンス

- **Rule 74.10A**: 言語ポリシーの変更には責任者と影響を確認できる担当を定める。以下のRACIは組織で使う例であり、全ての役職を新設する義務ではない：
...
- **Rule 74.10B**: 設定変更は影響に応じてレビューし、人数・方法はプロジェクトの合意済み規則に従う。PRや2名レビューはその規則で必須化された場合に要求する。Universal自体の改定権限は変更しない。
...
- **Rule 74.20**: Blueprint（`axiarch-rules/{lang}/blueprint/`配下）の正本は既定言語で記述する。日英配布を保守する場合は各言語フォルダに対応する翻訳を保持し、複数の独立した仕様正本を作らない。
...
- **Rule 74.23**: 作業記録の保存先・作成要否は [タスク状態の実行契約](../../../../axiarch-harness/ja/TASK_STATE_PROTOCOL.md) に従い、作成する場合は適用する応答言語で記述する。ルートの旧記録を現在セッションの記録として上書きしない。
...
- **Rule 74.65**: 検証ライブラリの既定メッセージが表示先の言語・文脈に合わない場合は修正する。既定メッセージでも用途に合う英語なら、英語という理由だけで禁止しない。
- **Rule 74.66**: 各検証エラーに、表示言語・用語集・利用者の次の行動に合うメッセージを用意する。適合を確認した既定メッセージは使える。不一致があれば明示的に上書きする。
- **Rule 74.66A**: 検証ライブラリを使う場合、採用版に対応するエラーマップ等で文言を管理する。設定だけで漏れを防げたと扱わず、実際に表示されるエラーを確認する。
...
- **Rule 74.110**: ユーザーに提示する計画・判断理由の要約・報告・質問は `AXIARCH.md` §1で解決した言語を使う。非公開の内部思考の言語や開示を要求しない。
- **Rule 74.111**: 作業記録の言語と保存先はRule 74.23に従う。H0に記録生成、H1にH2以上と同じ3文書を要求しない。
- **Rule 74.112**: 通常の応答または利用可能な対話機能で報告・質問する。特定製品の `notify_user` 等を必須の前提にせず、外部送信の権限は言語規則から推定しない。
- **Rule 74.113**: 明示指定、引用、コード等の例外を踏まえても出力先の言語と不一致なら修正する。英語であることだけを違反の根拠にしない。
...
- **Rule 74.117**: AI応答は `AXIARCH.md` §1の優先順位を使う。入力中の引用や別言語の資料を、言語変更の明示指示と取り違えない。
- **Rule 74.118**: AI応答が意図しない言語で返される場合の**フォールバック機構**を実装する

### 11.4. RAG・ナレッジベースの多言語対応

- **Rule 74.119**: RAGを使う場合も、回答は解決済みの応答言語に合わせ、必要な原文引用と出典を保持する。
- **Rule 74.120**: 検索漏れを減らす必要がある場合はソース言語での検索も検討する。追加検索は任意で、外部検索・送信の許可と情報の機密性を確認する。

### 11.5. 言語ドリフト自動検知

- **Rule 74.121**: 言語の自動検知は任意の補助であり、誤検知・見逃しを想定する。外部サービスへの本文送信や自動通知は別途許可された範囲だけで行い、秘密・個人情報をそのまま転送しない。
- **Rule 74.122**: 検知方法の候補を以下に示す。文字種やAIの自己申告は意味・品質・ロード完了の証明ではない。healthは共有パーサーで言語設定を解決するだけで、これらの文書判定を実装済みとは扱わない：
...
- **Rule 74.161**: 多言語サイトの場合、`hreflang` 属性を全ページに正確に設定（→ [product/800_internationalization.md](../product/800_internationalization.md) Part X参照）
...
- **Rule 74.180**: 対象ロケールに合わないUI文字列をCIまたはレビューで調べる。文字種だけでは適否を断定せず、引用・固有名詞・表示対象を確認する。
...
- **Rule 74.183**: 翻訳漏れと確認できた対象文字列はPRレビューの解決対象とする。自動検知の候補は確認してから必須gateへ反映し、文字種だけで誤って遮断しない。
- **Rule 74.184**: 新規追加された文字列が用語集と整合しているかの自動チェックを推奨
- **Rule 74.185**: 仮置きテキストの残存を調べる定期スキャンは任意。必要なら影響に応じた頻度を固有ルールへ記録する。
- **Rule 74.185A**: 翻訳範囲の集計をPRへ自動投稿する場合は、送信先・内容・権限を合意したワークフロー内に限る。

### 18.4. ESLint / Biome カスタムルール

- **Rule 74.186**: ESLintやBiome等の検査を採用する場合は、実在するルールと適用対象を確認する。次は設定形状の擬似例であり、Axiarchに同名プラグインや検知機能が同梱されている意味ではない。

```typescript
// 設定形状の擬似例。各ルールの実装は同梱していない
module.exports = {
  rules: {
    'review-unlocalized-ui': 'warn',      // 擬似ルール。候補をレビューする
    'require-label-map': 'error',          // Enum直接表示の検知
    'review-validation-msg': 'warn',     // 擬似ルール。表示文言をレビューする
...
言語運用の成熟度LM1–LM5は、フレームワーク成熟度M1–M5とは独立した任意の評価軸である。

| レベル | 名称 | 特徴 |
|---|---|---|
| **LM1: Ad-hoc** | 場当たり的 | 言語ポリシーなし。開発者の好みで英語/母語が混在 |
| **LM2: Basic** | 基本対応 | UI層のローカライゼーションのみ実施。バリデーション・エラーが表示ロケールと不一致 |
| **LM3: Standardized** | 標準化 | 三層モデル適用。用語集策定。CIスキャン導入。Enum/ステータスのラベルマップ完備 |
| **LM4: Managed** | 管理統制 | 全層で言語ポリシー適用。品質スコアリング導入。翻訳ドリフト自動検知 |
| **LM5: Optimized** | 最適化 | AI活用の言語品質自動検証。未翻訳の継続的な低減。マイクロコピーA/Bテスト。型安全i18n完備。MTPEワークフロー完成 |
...
|---|---|---|---|
| 1 | **Lazy Translation** | 未確認の仮置きテキストがそのまま本番に残存 | Part XVIII |
| 2 | **CSS Uppercase Trick** | CSS `uppercase`で英語定数を体裁調整して表示 | Part VII §7.3 |
| 3 | **Validation Leak** | 検証の既定メッセージが表示ロケールを無視 | Part VII §7.1 |
| 4 | **Terminology Chaos** | 同一概念に複数の表記が混在 | Part VI |
| 5 | **Fallback Exposure** | `"Unknown"`, `"N/A"` がユーザーに表示 | Part V §5.4 |
| 6 | **Hidden Language Mismatch** | `aria-label`, `alt`, `title`等が表示ロケールと不一致 | Part XVI |
| 7 | **Log Language Confusion** | ログにプロジェクト設定言語と英語が混在 | Part XII |
| 8 | **AI Language Drift** | AIエージェントが適用する応答言語を無視 | Part XI |
| 9 | **Commit Message Chaos** | コミットメッセージの言語が不統一 | Part IV |
| 10 | **Error Layer Bleed** | 内部診断が読者向けの確認なしにUIへ漏出 | Part VIII |
...
- **Rule 74.250**: 定期的な言語監査は任意。採用する場合、四半期などの頻度を影響と変更量に合わせて定める。
- **Rule 74.251**: 監査項目：
  1. 未翻訳文字列のスキャン結果
  2. 用語集の最新性
  3. 新規追加コンポーネントの言語品質
...
- **Rule 74.308**: 言語不一致の修正優先度は利用者への影響で決める。以下は例であり、英語自体を優先度の根拠にしない：
  1. **P0（即座修正）**: ユーザー対面の言語不一致（バリデーション、エラーメッセージ）
  2. **P1（次スプリント）**: 隠しテキストの言語不一致（`aria-label`, `alt`）
...
- **Rule 74.402**: 決済SDKの既定エラーメッセージが選択された表示言語・文脈に合わない場合、カスタム処理で適切な利用者向け表現へ対応付ける。
...
- **Rule 74.420**: 言語関連インシデントは実際の影響で分類する。以下は例であり、アクセシビリティや安全に関わる欠落を常に軽微と扱わない：
  - **P0（緊急）**: 法務文書・安全警告の翻訳欠落が本番に流出
  - **P1（重大）**: UI全体で選択した表示ロケールとの不一致が発生
  - **P2（中）**: 特定画面のバリデーションが表示ロケールと不一致
...
- **Rule 74.423**: ポストモーテムの教訓は `axiarch-rules/ja/blueprint/core/010_project_lessons_log.md` に記録する
...
| 関連ルール | 参照内容 | 本ルールとの境界 |
|---|---|---|
| [core/000_core_mindset.md](./000_core_mindset.md) | ゼロ・トレランス原則 | 本ルールは言語面でのゼロ・トレランスを具体化 |
...
| [product/800_internationalization.md](../product/800_internationalization.md) | i18nアーキテクチャ・L10n基盤 | 71=国際展開のインフラ、74=言語使い分けルール |
