...
> - **基本原則**: ユーザー・開発者間のコミュニケーションは、`AXIARCH.md` の `Project Native Language` で定められた言語で行わなければならない。旧導入先では `AGENTS.md` をフォールバックとして参照する。文化的文脈・ニュアンスを含めた「高度な言語運用」を要求する。
...
- **Rule 74.1**: 全プロジェクトは `AXIARCH.md` の `Project Native Language` で**一つの権威言語**を定義しなければならない。旧導入先では `AGENTS.md` をフォールバックとして扱う
...
- **Rule 74.9**: 言語切替時のUI状態（フォーム入力値・スクロール位置・モーダル状態）を確実に保持する
