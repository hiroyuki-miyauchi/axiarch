...
> - 基本原則: AIの応答言語の優先順位は、プロジェクトルートの `AXIARCH.md` §0を正本とする。ユーザーの最新の明示指定を優先し、指定がなければ `Project Native Language` を既定とする。英語を選んだ利用先では英語が正しい出力になる。
...
- **Rule 74.7**: 製品UIの言語選択では、利用者の現在の明示選択を優先する。以下は対応ロケール内での解決例であり、AI応答の優先順位は `AXIARCH.md` §0に従う。位置による推定は任意で、利用目的・許可範囲を確認し、言語選択だけのために精密位置情報を収集しない：
...
- **Rule 74.110**: ユーザーに提示する計画・判断理由の要約・報告・質問は `AXIARCH.md` §0で解決した言語を使う。非公開の内部思考の言語や開示を要求しない。
...
- **Rule 74.117**: AI応答は `AXIARCH.md` §0の優先順位を使う。入力中の引用や別言語の資料を、言語変更の明示指示と取り違えない。
