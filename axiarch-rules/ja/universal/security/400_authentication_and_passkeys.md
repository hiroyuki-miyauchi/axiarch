...
### 1.3. CIAM と Workforce IAM の設計分岐

-   **Law**: 対象が**コンシューマ（CIAM: Customer Identity & Access Management）**か**従業員／組織内（Workforce IAM）**かで設計判断が分岐する。同一方針を両者に流用してはならない（MUST）。利用シーンを最初に特定し、本ファイルの要件をその区分に合わせて適用する。

| 観点 | CIAM（コンシューマ） | Workforce IAM（従業員・組織） |
|:-----|:--------------------|:------------------------------|
| **リカバリー** | セルフサービス前提（自動・低摩擦）。最弱リンク防御（§9）が特に重要 | 管理者主導の復旧（ヘルプデスク・ID証明）が可能。本人確認(IAL)を厳格化 |
| **MFA厳格度** | 段階導入・摩擦最小化（離脱回避）。フィッシング耐性へ漸進移行（§7） | 例外なく強制可能。管理者・特権は Device-Bound 必須（§5.1） |
| **プロビジョニング** | セルフサインアップ＋登録時不正対策（§8.1b） | SSO/JIT/SCIM による集中管理（→`410` §10） |
| **監査保持期間** | プライバシー法（データ最小化・保持期限）優先（`axiarch-rules/{lang}/universal/security/100_data_governance.md`） | 内部統制・コンプライアンス要件で長期保持が必要な場合がある |
| **規模/匿名性** | 大規模・匿名トラフィック・ボット混入前提 | 既知の有限ユーザー集合 |

-   登録時不正・ボット対策（§8.1b）は特に CIAM で重要となる。
...
> **参考規格**: W3C WebAuthn **Level 2 (REC=勧告)**, WebAuthn **Level 3 (Candidate Recommendation; RECではない)**, FIDO2, CTAP2.1, FIDO Alliance Passkey Guidelines
...
-   **Rule 64.3.3b（同期 vs デバイスバインドの選択ポリシー）**: 同期かデバイスバインドかを**アカウント区分・操作ティアでポリシー化**し、その場任せにしない（MUST）。最低限、(a) 一般ユーザー＝Synced 既定、(b) 管理者・特権・高保証＝Device-Bound 必須（§3.2 Rule 64.3.2）、(c) コンプライアンスで単一デバイス保証が要る場合＝Device-Bound、を明文化する。受領した BE/BS フラグでポリシー適合を**サーバ側で強制**し、要件未満（例: 管理者が Synced のみ）の登録を拒否する。
...
-   **採用方針**: WebAuthn **Level 2 を REC（勧告・安定）基盤**として実装し、Level 3 の機能（Related Origin Requests, Signal API, `getClientCapabilities()` 等）は**プログレッシブエンハンスメント**として、機能検出（feature detection）の上で追加する。
-   **規格ステータスの正確化**: WebAuthn Level 3 は **Candidate Recommendation（CR; REC＝勧告には未到達）**であり、まだ変更余地がある。L3 機能の非対応をエラーにせず、**REC である L2 のコア機能で必ず認証が完結**する設計にする（MUST）。

### 3.12. パスキープロバイダの信頼モデル

-   **背景**: Synced Passkey の秘密鍵は同期プロバイダ（パスワードマネージャ）の管理下で複製・同期される。したがって**プロバイダの信頼性が認証強度の前提**になる。プロバイダによって鍵保護・E2E暗号化・復旧手段・監査の水準は異なる。
-   **Rule 64.3.13（プロバイダ信頼度の評価）**: パスキープロバイダを信頼度で区分し、用途に応じて要求水準を定める。
    -   **高信頼（既定許容）**: OSプラットフォーム提供（Apple iCloud Keychain / Google Password Manager / Microsoft）。同期鍵の保護・E2E が組み込まれ、エコシステム統合が前提。
    -   **要評価（個別判断）**: サードパーティのパスワードマネージャ（1Password 等）。鍵のE2E暗号化・復旧・組織管理の各仕様を評価したうえで許容する。
    -   高保証用途では同期プロバイダ強度に依存しない Device-Bound（§3.2）を優先する。
-   AAGUID で認証器／プロバイダ種別を識別できる場合は、用途に応じて許可プロバイダのポリシーを定義する（過度なロックインは避け、ユーザー選択肢とのバランスを取る）。

### 3.13. クレデンシャル可搬性（CXP / CXF — 将来対応）

-   **背景**: FIDO Alliance は 2025 年に **Credential Exchange Protocol (CXP) / Credential Exchange Format (CXF)** を公開し、パスキーをプロバイダ間で**安全にエクスポート／インポート**する標準化を進めている。従来のプロバイダロックイン（移行不能）を緩和する方向。
-   **Rule 64.3.14（将来対応の認識）**: CXP/CXF は策定・実装が進展途上のため**現時点で新規必須要件ではない**が、(a) ユーザーのプロバイダ移行を妨げる設計を避ける、(b) クレデンシャルメタデータの移行可能性を確保する（§11.2 と整合）、(c) 移行（インポート）されたパスキーの信頼評価方針を将来の設計拡張点として認識する。標準の成熟に応じて対応を見直す。
...
-   **Rule 64.8.1b（登録時のボット／不正対策）**: 特に CIAM（§1.3）のセルフサインアップでは、アカウント大量生成・偽アカウント・新規登録時の不正を抑止する。
    1.  **クライアント信号はサーバ側検証必須（MUST）**: デバイス／アプリ完全性シグナル（Android **Play Integrity**、Apple **App Attest**、Web の **Cloudflare Turnstile** / reCAPTCHA 等）は、トークンを**サーバ側で発行元へ検証**して初めて信頼する。クライアントが返す合否や属性をそのまま信頼してはならない（改ざん・リプレイ可能）。
    2.  これらは**補助シグナル**であり、単独の認証要素にしない（420 §9 のフィンガープリント原則と同様）。リスクスコア（→`420` §8）の入力として統合する。
    3.  登録（送信）エンドポイントにレート制限・列挙防止を課す（§9.6）。SMS を絡める場合は Toll Fraud（§10.2）にも留意。
    4.  完全性検証の失敗を即ブロックにせず、リスクに応じた段階的摩擦（追加検証）に倒し、正規ユーザーの誤検知離脱を避ける。
...
| 23 | 同期/デバイスバインドをポリシー化せずサーバ側強制しない | BE/BS でポリシー適合をサーバ側強制（§3.2 64.3.3b） |
| 24 | Play Integrity/App Attest/Turnstile の合否をクライアント任せで信頼 | トークンをサーバ側で発行元検証（§8.1b） |
| 25 | 完全性／ボット信号を単独の認証要素にする | 補助シグナルに限定しリスクスコアへ統合（§8.1b） |
| 26 | CIAM と Workforce IAM に同一設計を流用 | 利用区分で設計分岐（§1.3） |
...
| Synced vs Device-Bound / backupEligible / BE/BS フラグ / 選択ポリシー | §3.2 |
| パスキープロバイダ信頼モデル / 同期プロバイダ / AAGUID | §3.12 |
| CXP / CXF / クレデンシャル可搬性 / プロバイダ移行 | §3.13 |
| CIAM / Workforce IAM / 設計分岐 / リカバリー / 監査保持 | §1.3 |
| 登録時不正 / ボット対策 / Play Integrity / App Attest / Turnstile | §8.1b |
