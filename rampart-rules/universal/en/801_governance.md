...
> - This document serves as the "Meta-Constitution" for the entire Rampart rule system.
...
*   **Definition**: All rule files under `rampart-rules/universal/` are collectively referred to as the "**Constitution**".
...
*   **Sole Authoritative Source**: The only authoritative source for rules is version-controlled Markdown files within the `rampart-rules/` directory.
...
*   **Centralization Obligation**: All rules, policies, and constraints must be consolidated as version-controllable Markdown files within `rampart-rules/`.
...
> - **Requirement**: Before executing any task, relevant rules within `rampart-rules/` must be referenced and understood.
...
*   **AI-Assisted Analysis Examples**:
    ...
    grep -r "§12" --include="*.md" rampart-rules/
    
    # Detect cross-reference impacts
    grep -r "72_constitution" --include="*.md" rampart-rules/
    
    # Verify ja/en sync status (section count comparison)
    diff <(grep -c "^### " rampart-rules/universal/ja/801_governance.md) \
         <(grep -c "^### " rampart-rules/universal/en/801_governance.md)
    ```
...
*   **Law**: Design documents within `rampart-rules/blueprint/` are not "reference materials" but are **part of the code (Logic)**.
...
*   **Verification Command Example**:
    ...
    git log --show-signature -- rampart-rules/universal/en/801_governance.md
    ```
...
*   **CODEOWNERS**: Set up CODEOWNERS for rule files to prevent changes without approval.
    ```
    # .github/CODEOWNERS
    /rampart-rules/universal/ @project-owner
    ```
...
*   **Technical Prohibition of `git push --force`**:
    ...
    #!/bin/bash
    # Detect and reject force push to rampart-rules/
    while read oldrev newrev refname; do
      if [ "$oldrev" != "0000000000000000000000000000000000000000" ]; then
        forced=$(git rev-list "$newrev...$oldrev" -- rampart-rules/ | wc -l)
        if [ "$forced" -gt 0 ]; then
          echo "ERROR: Force push to rampart-rules/ is prohibited (Rule 72/§1.2)"
          exit 1
        fi
      fi
    done
...
*   **Rule Manifest**:
    ...
    find rampart-rules/ -name "*.md" -type f | sort | while read f; do
      ...
      validate:
        ...
        steps:
          ...
          - name: Check naming convention
            run: |
              find rampart-rules/ -name "*.md" | while read f; do
                basename="$(basename "$f")"
          ...
          - name: Check bilingual sync
            run: |
              ja_count=$(find rampart-rules/universal/ja/ -name "*.md" | wc -l)
              en_count=$(find rampart-rules/universal/en/ -name "*.md" | wc -l)
...
*   **Recovery from Git History**:
    ...
    git checkout <commit-hash> -- rampart-rules/universal/en/801_governance.md
...
*   **Rule System Protection in MCP Integration**:
    1.  **Prohibition of Rule Exposure as MCP Server**: Directly exposing rule files under `rampart-rules/` as MCP Resources to external parties is **prohibited in principle** due to context pollution and misinterpretation risks. Reading must always apply the autonomous selection protocol (§3.2) via INDEX.md
...
*   **Location**: Provided in `rampart-rules/README.md`.
...
*   **Search Command Example**:
    ...
    grep -rl "security" --include="*.md" rampart-rules/universal/
    ```
...
*   **Initialization Flow**:
    ...
    2.  Read `rampart-rules/INDEX.md` to understand the overall structure
...
|---|---|---|
| **Total Line Count** | Total lines across all `rampart-rules/` | > 10,000 lines |
