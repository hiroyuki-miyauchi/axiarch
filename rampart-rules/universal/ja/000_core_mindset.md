...
>
> 1.  本ドキュメント群 (`rampart-rules/universal/*.md`) は、本プロジェクトの開発・運用・ビジネスにおける**最高法規**です。
...
> この「Core Philosophy」は、Google Rampartの全ての活動における憲法であり、例外は認められません。
...
2. [§1. Rampart（ランパート）マインドセット](#1-rampartランパートマインドセット-the-rampart-mindset)
...
6. [§6. シリコンバレーDNA](#6-シリコンバレーdna-silicon-valley-dna)
7. [§7. 開発・運用の鉄則](#7-開発運用の鉄則-development--operations-iron-rules)
8. [§8. グローバル・ガバナンス・プロトコル](#8-グローバルガバナンスプロトコル-global-governance-protocols)
9. [Appendix A: 逆引き索引](#appendix-a-逆引き索引)
...
## 1. Rampart（ランパート）マインドセット (The Rampart Mindset)
...
*   **言語選択 (Language Selection)**:
    ...
    *   **ルール適用**: AIは `AGENTS.md` の設定に従い、対話言語と言語ルールを適用します。プロジェクト初期化時に、不要な方の言語ディレクトリ（`rampart-rules/universal/` および `rampart-rules/blueprint/` 内）を削除してください。
...
*   **Universal Rules (不変)**: `rampart-rules/universal/` は、Google RampartのDNAです。プロジェクトごとの勝手な変更は許されません。
*   **Blueprint Rules (可変)**: プロジェクト固有の事情は `rampart-rules/blueprint/` で管理します。
...
*   **Emergency & Compliance**: 以下に該当する場合のみ、保護規定の例外として修正案を最優先で作成・提示し、ユーザーの即時承認を得て対処してください（独断実行は禁止）:
    ...
    *   **Constitution Violation**: Rampart憲法への重大な違反。
...
*   **Law 2 (Husky Guard)**: 全てのプロジェクトにおいて、**Husky** の導入および `pre-push` フックによる `main` ブランチへの直接プッシュ禁止設定を **義務（Universal Mandate）** とします。「気をつける」という運用ルールは無意味であり、コードによる物理的な防衛線のみを信頼します。
    *   **Implementation**: 具体的なセットアップ手順および技術詳細については、`300_engineering_standards.md` を参照してください。
*   **Action**:
...
|---|---|
| §0 優先順位の階層 | `600_security_privacy`, `601_data_governance`, `200_design_ux`, `720_cloud_finops` |
| §1 マインドセット | `300_engineering_standards`, `700_qa_testing` |
| §2 エリート・ロール | `100_product_strategy`, `101_revenue_monetization` |
| §3 言語基準 | `802_language_protocol` |
| §4 ガバナンス | `801_governance` |
| §7 開発・運用の鉄則 | `300_engineering_standards`, `200_design_ux` |
| §8 グローバル・ガバナンス | `300_engineering_standards`, `502_site_reliability` |
