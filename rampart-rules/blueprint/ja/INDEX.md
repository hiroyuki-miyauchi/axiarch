...
- [998. Feature Specification Template](998_feature_spec_template.md) - **機能仕様テンプレート（SDD核）**。受け入れ条件・データモデル・API設計・テスト戦略を機能単位で定義する。Category 600–800 にコピーして使用。
- [999. Project Specific Template](999_project_specific_template.md) - プロジェクト固有ルールファイル追加時のテンプレート。
...
1. **`998_feature_spec_template.md` をコピーする**
2. Category 600–800 の番号帯でファイル名を変更する（例: `600_feature_payment.md`）
3. **§3 受け入れ条件（Acceptance Criteria）を最優先で記述する** — このセクションが空の状態でコードを書いてはならない
4. 残りのセクション（データモデル、API設計、テスト戦略等）を埋める
5. **このファイル（INDEX.md）の Category 600–800 にエントリを追記する**

### プロジェクト固有ルールの追加
1. `999_project_specific_template.md` をコピーする
2. 上記のカテゴリ番号帯に従ってファイル名を変更する（例: `100_security_policy.md`）
...
- `AGENTS.md` の「Continuous Improvement」プロセスに基づく義務的な振り返り
...
| 機能仕様テンプレート・受け入れ条件・Given/When/Then | `998_feature_spec_template.md` |
| プロジェクト固有ルールテンプレート | `999_project_specific_template.md` |
