...
>
> 1. 新規ドメインファイル（`0X0_lessons_{domain}.md`）を作成
> 2. 該当する教訓を新ファイルに移動
> 3. このファイルにドメインファイルへの参照リンクを追記
>
> **操縦者（ユーザー）は何もする必要がありません。** AIが自動的にイヌコミのようなドメイン別構造を構築します。
...
<!-- 例: | 020 | DB・認証 | [020_lessons_database_auth.md](./020_lessons_database_auth.md) | 5 | -->
...
**Rule:** `AGENTS.md` および `rampart-rules` のプロトコルを絶対的な真実として遵守すること。
...
| パフォーマンス | 速度改善、メモリ、コスト最適化 | `300_engineering`, `720_finops` |
