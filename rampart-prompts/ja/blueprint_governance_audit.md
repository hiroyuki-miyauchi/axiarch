...
>
> **対象**: `rampart-rules/blueprint/` 配下のプロジェクト固有ルール
>
...
特に `rampart-rules/universal/` 配下のルールは「物理法則」であり、今回のタスクでの変更は許可されません。変更はプロジェクト固有の `blueprint` 側でのみ行ってください。
...
2.  **Dynamic Rule Discovery (階級別ロード)**:
    * `rampart-rules/` ディレクトリ配下をスキャンし、以下の**2階級（Class）**に厳密に分類してロードせよ。
...
* **Target Path**: `rampart-rules/universal/` 内の全ファイル。
...
* **Target Path**: `rampart-rules/blueprint/` 内の全ファイル。
...
2.  **Governance Audit (現状分析)**:
    ...
    * **Execution Standards Check**: 現在のルール群が、**上記20の観点（特に日本語最適化、AI最適化、GEO、LTV、FinOps）**を網羅できているか監査する。
...
解析結果に基づき、以下の3つの柱で **Class A (Blueprint)** ルール群を徹底的にブラッシュアップしてください。
...
2.  **Plan (構成案)**:
    * `rampart-rules/blueprint/` 内の理想的なファイル構成（ファイル名と役割など）を定義する。**3桁Sparse Numbering（間隔採番）**を適用し、将来の拡張余地を確保する。
...
**これより、プロジェクトの全知を結集し、既存の資産（ルール）を劣化させることなく、固有ルール全体（`rampart-rules/blueprint/*.md`）の徹底的な最適化と再構築を開始してください。**
````
* **Next Action**: 開発者がこのガバナンス・アーキテクチャをどう活用し、運用していくべきかの指針。
