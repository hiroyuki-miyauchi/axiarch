...
>
> **対象**: プロジェクト全体（ソースコード + `rampart-rules/`）
>
...
2.  **Target 1: The Constitution (最高憲法)**
    ...
    * **Class S (Universal)**: `rampart-rules/universal/` 配下 → **Read-Only（変更禁止）**
    * *Search Hint*: `000_`, `600_security`, `constitution`
3.  **Target 2: Project Lessons (プロジェクト教訓)**
    * **役割**: 過去の失敗や絶対禁止事項が記録されたログファイル。
    * **Class A (Blueprint)**: `rampart-rules/blueprint/` 配下 → **Read/Write（更新対象）**
