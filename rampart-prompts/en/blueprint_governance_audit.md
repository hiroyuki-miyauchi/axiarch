...
>
> **Target**: Project-specific rules under `rampart-rules/blueprint/`
>
...
Rules under `rampart-rules/universal/` are "laws of physics" and MUST NOT be modified in this task. Changes are ONLY permitted in the project-specific `blueprint` directory.
...
In the rule-making process, you must think deeply and comprehensively across **all standards defined in `AGENTS.md` and the loaded rule set (SSOT)**. Proactively identify and propose additions/improvements if the governance architecture has gaps in "market fit," "business opportunity," or "operational risk."
...
2.  **Dynamic Rule Discovery (Autonomous Loading Specification)**:
    * Target the `rampart-rules/` directory, **autonomously select files in strict compliance with the `AGENTS.md` directives and the `LOADING_PROTOCOL.md` "Anti-Laziness Rule"**, and strictly classify into the following **2 Classes.**
...
* **File Naming & Structuring**: Create new files strictly abiding by `CRYSTALLIZATION_PROTOCOL.md` and existing file naming conventions to ensure room for future expansion.
...
2.  **Plan**:
    * Define the ideal file structure in `rampart-rules/blueprint/` (filenames and roles). Apply the `CRYSTALLIZATION_PROTOCOL.md` naming conventions to ensure room for future expansion.
...
**Begin the thorough optimization and reconstruction of all project-specific rules (`rampart-rules/blueprint/*.md`), leveraging the full knowledge of the project without degrading existing assets (rules).**
````
