...
2.  **Dynamic Rule Discovery (Complete Rule Hierarchy Mastery)**:
    ...
    * **Class S: Universal (Immutable)**:
        * All files under `rampart-rules/universal/`. **Modification prohibited under all circumstances (Read-Only).**
    * **Class A: Blueprint (Project-Specific / Editable)**:
        * All files under `rampart-rules/blueprint/`. **Subject to updates and additions based on fix results (Read/Write).**
...
* **Rule Update Proposal**:
    * If "lessons" or "new implementation rules (e.g., type definition handling)" were gained through this error fix, present proposals for additions/modifications to **relevant files in `rampart-rules/blueprint/`.**
    * **Modification Prohibited**: `AGENTS.md` and `rampart-rules/universal/` are the absolute constitution — NOT subject to change proposals.
...
Constitution (AGENTS.md) and all rule profiles loaded.
...
Upon presentation, will immediately initiate Phase 1 (Reproduction & Root Cause Analysis),
