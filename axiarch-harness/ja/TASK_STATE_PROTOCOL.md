...
hook入力に `cwd` がある場合、記録・補足はその作業先から最も近い `AXIARCH.md` を探して解決する。起動時環境変数が元のcheckoutを指す場合も、現在の作業コピーを使う。解決不能は警告として記録生成を止め、別checkoutへ代替しない。製品別条件は `axiarch-scripts/AGENT_COMPATIBILITY.md` を参照する。
...
更新確定時はコピー済み・変更なしの配布ファイルを更新元と再照合し、診断中の変更を正常版数や比較元として扱わない。保留・確認不能の結果と復旧手順は `axiarch-scripts/README.md` の「初期導入と任意生成の結果」を参照する。
