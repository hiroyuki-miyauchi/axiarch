...
## 6. Flutter統合 (Flutter Integration - Platform Channels)
### 6.1. 型安全性 (Type Safety - Pigeon)
...
## 9. Web-Native統合 (Web-Native Integration - Bridge & PWA)
*   **Mobile Bridge Protocol**:
    *   **JS Interface**: ネイティブ機能（ハプティック、Pushトークン）を呼び出す規格を `window.AppBridge` で統一します。
    *   **User Agent**: アプリからのアクセスをUA (`NativeApp/1.0`) で判別し、Web用ヘッダー/フッターを非表示にします（二重ナビゲーション防止）。
*   **App Links & PWA**:
    *   **Deep Links**: `/.well-known/apple-app-site-association` を配置し、URLがネイティブアプリで直接開くように構成します。
    *   **PWA**: `manifest.json` を整備し、モバイルWebユーザーにも「ホーム画面に追加」時のUX（Standalone表示）を保証します。

## 10. セキュリティとAPIプロトコル (Security & API Protocols)
*   **The Native Bypass Protocol (No Secrets in Binary)**:
    *   **Law**: ネイティブアプリ（IPA/APK）はデコンパイル可能です。API Key (`x-api-key`) をバンドルすることはセキュリティリスクです。
    *   **Action**: ユーザー認証済みのリクエストには `Authorization: Bearer <token>` のみを付与し、API Keyを使用しないでください。バックエンドはこのトークンによりVIPレーン（Tier 2 Access）を提供します。
