...
[![Rules](https://img.shields.io/badge/Universal_Rules-46_files-green.svg)](#-universal-rules-46-files--jaen)
...
│ └─ Universal Rules (46ファイル: 不変の普遍的基準)               │
├─────────────────────────────────────────────────────────────┤
...
│ └─ Universal Rules (46 files / Immutable Universal Standards) │
├───────────────────────────────────────────────────────────────┤
...
### 📚 Universal Rules (46 files × JA/EN)
...
| Engineering | 11 | コード品質(80§)、API、Supabase、Web、CMS、Flutter、Native、Firebase、AWS、**Git Workflow**、**バッチ・バックフィル・失敗計数** | Code quality (80§), API, Supabase, Web, CMS, Flutter, Native, Firebase, AWS, **Git Workflow**, **batch/backfill & failure accounting** |
...
成果：数千規模の憲法基準を扱う46のUniversalルールファイル、ハルシネーションリスクを軽減する5ステップのBoot Sequence Protocol、教訓をルール化しやすくするCrystallization Protocol。現行構成では `AXIARCH.md` を正本入口として参照します。
