...
> [!IMPORTANT]
> **JA**: `main` ブランチの `init.sh` は Axiarch v1.11.0 を導入します。既定では `tags/v1.11.0` を取得し、別バージョン固定が必要な場合は該当リリースタグを指す `AXIARCH_REF=tags/vX.Y.Z` を右辺の `bash` に渡してください。
>
> **EN**: The `main`-branch `init.sh` installs Axiarch v1.11.0. By default it fetches `tags/v1.11.0`; to pin another release, pass `AXIARCH_REF=tags/vX.Y.Z` to the right-hand `bash` process.
