...
AI支援開発における品質ブレ・ハルシネーション・暴走を、第1層「Universal（不変の普遍憲法）」・第2層「Blueprint（動的成長する固有仕様）」・第3層「Prompts（任意実行トリガー）」からなる **3層ガバナンス・アーキテクチャ** で強固に抑止・統制します。
...
│ └─ Universal Rules (38ファイル: 不変のエンジニアリング基準)      │
├─────────────────────────────────────────────────────────────┤
...
│ └─ Universal Rules (38 files / Immutable Eng. Standards)    │
├─────────────────────────────────────────────────────────────┤
...
成果：2,500以上のエンジニアリング基準を網羅する38のUniversalルールファイル、ハルシネーションを排除する5ステップのBoot Sequence Protocol、教訓を自動的にルールに変換するCrystallization Protocol。すべて、たった一つの `AGENTS.md` 憲法を通じて強制されます。
...
- **役割**: 時代やプロジェクトが変わっても揺るがない「エンジニアリングの絶対的真理と制約」を定義する岩盤層（`AGENTS.md` + `universal/`）。
- **特性**: **Read-Only（強制保護）**。AIエージェントによる独断の変更を固く禁じた「不変の領域」です。AI特有の独自解釈（Vibe Coding）やセキュリティ脆弱化を物理的に阻止します。

#### Layer 2: Blueprint (可変層 / Mutable Project State)
- **役割**: プロジェクトの事業目的、固有の機能仕様、そして開発中に得られた「実践の教訓」を蓄積・結晶化する層（`blueprint/`）。
- **特性**: **Read-Write（動的成長）**。AIエージェント自身が継続的に仕様や教訓を書き込み、自律ロードを通じて文脈を復元するための「生きた記憶領域」です。「昨日までの前提を忘れる（Context Amnesia）」リスクを大幅に軽減し、継続的な品質の底上げ（Quality Floorの向上）を実現します。

#### Layer 3: Prompts (任意層 / Optional Execution Engine)
- **役割**: Layer 1と2のルールを、特定タスク（セキュリティ監査、設計最適化、インシデント対応など）へ強制的に呼び出し適用させるエンジン（`axiarch-prompts/`）。
- **特性**: **Optional（任意導入）**。ルールがあっても守られない「形骸化」を防ぐトリガーとして機能します。
...
> **JA**: Axiarch（アクシアーク）の卓越性は、単なるプロンプトの集合体ではなく、この「**普遍（Layer 1）と可変（Layer 2）の分離**」による状態管理アーキテクチャにあります。「Blueprint First（実装前の仕様定義の義務化）」と「憲法」による強力な統治を通じて、AI行動・品質・運用を一体化させた本質的なガバナンスを提供します。
> 
> **EN**: Axiarch's excellence lies not in being a mere prompt collection, but in its state-management architecture via "the **Separation of Immutable (Layer 1) and Mutable (Layer 2)**". Through "Blueprint First" and powerful constitutional governance, it provides fundamental architecture unifying AI behavioral, quality, and operational governance.
