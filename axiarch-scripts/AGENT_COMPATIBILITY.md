...
補足が見えない・省略された・時間切れになった場合は、[補足の受領確認](README.md#補足の受領確認--checking-reminder-delivery) に従います。宣言・生成・製品側受領・実読込を区別し、AIが確認可能な情報を先に調べます。設定の緩和や別セッションの新規作成を自動の復旧方法にしません。
...
For missing, shortened or timed-out context, follow [reminder delivery checks](README.md#補足の受領確認--checking-reminder-delivery). Distinguish declaration, generation, product delivery and actual reading; the agent investigates accessible evidence first. Relaxing settings or creating a different session is not an automatic recovery method.
