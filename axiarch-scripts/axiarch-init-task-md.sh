...
[AXIARCH SESSION START] This project enforces axiarch's BOOT SEQUENCE protocol on session start. Per AXIARCH.md, H2+ work records actual read paths/ranges in session task.md; H0 requires no durable task record and H1 uses a short record. Templates and reminders do not prove loading. task.md / implementation_plan.md / walkthrough.md are current-task Markdown evidence, not the native task/plan UI state. In Codex, when available, also call update_plan and keep exactly one in_progress step while work is active. In Claude Code, also use TaskCreate / TaskUpdate / TaskList / TaskGet when available; fall back to TodoWrite only in older runtimes without Task tools. / 本プロジェクトはセッション開始時に axiarch BOOT SEQUENCE を強制します。AXIARCH.md に基づき、H2以上は実際に読んだパスと範囲をセッションのtask.mdへ記録します。H0は永続記録不要、H1は短い記録で足ります。テンプレートや補足の表示はロード完了の証明ではありません。task.md / implementation_plan.md / walkthrough.md は現在タスク用のMarkdown証跡であり、ネイティブなタスク・プランUI状態ではありません。Codexでは利用可能な場合 update_plan も併用し、作業中は in_progress を1件だけ維持してください。Claude Codeでは利用可能な場合 TaskCreate / TaskUpdate / TaskList / TaskGet を併用し、Task tools がない古いランタイムでのみ TodoWrite にフォールバックしてください。
EOF
