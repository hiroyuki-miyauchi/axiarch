...
[AXIARCH BOOT] This project enforces axiarch governance. Before responding, the AI must load and apply AXIARCH.md (canonical protocol) and the LOADING_PROTOCOL.md BOOT SEQUENCE. Tool adapters such as AGENTS.md point to AXIARCH.md and are not separate rule sources. Output language follows the Project Native Language declared in AXIARCH.md and is applied to every heading, summary, label, list, table, and bullet in the response; when that language is Japanese, emitting English headings, summaries, labels, or section titles is a protocol violation (code, APIs, logs, paths stay in their required language). Rule files actually opened and read by the AI (AXIARCH.md, INDEX.md, LOADING_PROTOCOL.md, etc.) must be recorded in task.md; missing or stale load records are treated as protocol violations. task.md / implementation_plan.md / walkthrough.md are current-task Markdown evidence, not native task/plan UI state. When available, Codex must also update_plan, and Claude Code must also use TaskCreate / TaskUpdate / TaskList / TaskGet, with TodoWrite only as an older-runtime fallback. For non-trivial tasks (L2+), the Execution Harness in axiarch-harness/ is mandatory: run role passes, produce an audit verdict and an evidence packet, and stop at the human approval gate; read-only subagent delegation is not a human approval gate by itself, and named read-only workflows such as Deep Security Scan include their required worker fanout when explicitly requested; final judgment stays with the main agent. On task completion, the AI runs CRYSTALLIZATION_PROTOCOL Step 5 THRESHOLD CHECK and, when any domain in core/010_project_lessons_log.md holds 3+ unsorted lessons, promotes them into a dedicated Blueprint file before declaring the task done — appending to core/010 alone is not completion. / 本プロジェクトは axiarch ガバナンスを採用しています。応答前に、AI は AXIARCH.md（正本プロトコル）と LOADING_PROTOCOL.md の BOOT SEQUENCE を読み込んで適用する必要があります。AGENTS.md などのツールアダプターは AXIARCH.md への入口であり、別のルール正本ではありません。応答言語は AXIARCH.md で宣言された Project Native Language に従い、応答中のすべての見出し・要約・ラベル・箇条書き・表に適用されます。その言語が日本語の場合、英語の見出し・要約・ラベル・節タイトルを出すことはプロトコル違反です（コード・API・ログ・パスは必要な言語のまま）。AI が実際に開いて読んだルールファイル (AXIARCH.md / INDEX.md / LOADING_PROTOCOL.md 等) は task.md に記録する必要があります。未記録または実態と一致しないロード記録はプロトコル違反として扱われます。task.md / implementation_plan.md / walkthrough.md は現在タスク用のMarkdown証跡であり、ネイティブなタスク・プランUI状態ではありません。対応ランタイムでは Codex は update_plan、Claude Code は TaskCreate / TaskUpdate / TaskList / TaskGet も併用し、TodoWrite は古いランタイム向けのフォールバックに限定します。非自明なタスク（L2 以上）では axiarch-harness/ の Execution Harness が必須です。ロールパスを実行し、監査判定と証跡パケットを出し、人間承認ゲートで停止してください。読み取り専用のサブエージェント委任はそれ自体では人間承認ゲートではありません。Deep Security Scan などの名前付き読み取り専用 workflow が明示された場合、必要な worker fanout はその要求に含まれます。最終判断はメインエージェントが保持します。タスク完了時は CRYSTALLIZATION_PROTOCOL Step 5 THRESHOLD CHECK を実行し、core/010_project_lessons_log.md のドメインに 3 件以上の未整理教訓がある場合は Blueprint 専用ファイルへ昇華してから完了を宣言します。core/010 への追記だけでは完了とみなされません。
EOF

# v1.6.0+ short-circuit reminder (used after TTL window when no violations)
read -r -d '' SHORT_REMINDER <<'EOF' || true
[AXIARCH REMINDER] axiarch governance applies to this project. Continue applying AXIARCH.md / LOADING_PROTOCOL. Respond in the Project Native Language declared in AXIARCH.md — every heading, summary, label, list, table, and bullet (code, APIs, logs, paths excepted); when it is Japanese, English headings/labels are a protocol violation. For non-trivial tasks (L2+), apply the Execution Harness (axiarch-harness/): role passes, audit verdict, evidence packet, and stop at the human approval gate. Read-only subagent delegation is not a human approval gate; explicit Deep Security Scan requests include required read-only fanout. Keep native task/plan state in sync when available. Full reminder reappears on TTL expiry, violation detection, or new session. / axiarch ガバナンスは本プロジェクトに適用されます。AXIARCH.md / LOADING_PROTOCOL の適用を継続。応答は AXIARCH.md の Project Native Language で行い、すべての見出し・要約・ラベル・箇条書き・表に適用する（コード・API・ログ・パスは例外）。日本語の場合、英語の見出し・ラベルはプロトコル違反です。非自明なタスク（L2 以上）では axiarch-harness/ の Execution Harness（ロールパス・監査判定・証跡パケット・人間承認ゲート）を適用してください。読み取り専用のサブエージェント委任は人間承認ゲートではありません。明示された Deep Security Scan は必要な読み取り専用 fanout を含みます。利用可能ならネイティブなタスク・プラン状態も同期してください。TTL 期限切れ・違反検出・新規 session 時に full reminder が再表示されます。
EOF
