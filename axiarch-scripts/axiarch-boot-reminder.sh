...
# -----------------------------------------------------------------------------
# Static base reminder (bilingual; kept in this script, referenced from
# the configured hooks in .claude/settings.json or .codex/hooks.json before externalization)
# -----------------------------------------------------------------------------
read -r -d '' CORE_REMINDER <<'EOF' || true
[AXIARCH BOOT] This project uses Axiarch governance; this hook supplies a reminder, not proof of reading. Before non-trivial changes or verdicts, the AI must load and apply AXIARCH.md (canonical protocol) and the axiarch-rules/{lang}/LOADING_PROTOCOL.md BOOT SEQUENCE. Tool adapters such as AGENTS.md point to AXIARCH.md and are not separate rule sources. Follow the latest explicit user language instruction, then the Project Native Language declared in AXIARCH.md and is applied to every heading, summary, label, list, table, and bullet in the response; when that language is Japanese, emitting English headings, summaries, labels, or section titles is a protocol violation (code, APIs, logs, paths stay in their required language). For H2+, record actual paths and read ranges in the session task.md after reading. H0 needs no durable record; H1 needs only a short record. A missing recognizable row is a review hint, not proof of missing reads. task.md / implementation_plan.md / walkthrough.md are current-task Markdown evidence, not native task/plan UI state. When available, Codex must also update_plan, and Claude Code must also use TaskCreate / TaskUpdate / TaskList / TaskGet, with TodoWrite only as an older-runtime fallback. For non-trivial tasks (H2+), the Execution Harness in axiarch-harness/ is mandatory: run role passes, produce an audit verdict and an evidence packet, and apply the human approval gate in axiarch-harness/{lang}/HUMAN_APPROVAL_GATE.md; pause only if required approval is missing; read-only subagent delegation is not a human approval gate by itself, and named read-only workflows such as Deep Security Scan include their required worker fanout when explicitly requested; final judgment stays with the main agent. When creating or updating lessons, follow axiarch-rules/{lang}/CRYSTALLIZATION_PROTOCOL.md Steps 1–6, including count and age triggers and index updates. H0 read-only work needs no lesson cleanup. / 本プロジェクトは axiarch ガバナンスを採用しています。非自明な修正や判定の前に、AI は AXIARCH.md（正本プロトコル）と axiarch-rules/{lang}/LOADING_PROTOCOL.md の BOOT SEQUENCE を読み込んで適用する必要があります。AGENTS.md などのツールアダプターは AXIARCH.md への入口であり、別のルール正本ではありません。ユーザーの明示言語指定を最優先し、それ以外では応答言語は AXIARCH.md で宣言された Project Native Language に従い、応答中のすべての見出し・要約・ラベル・箇条書き・表に適用されます。その言語が日本語の場合、英語の見出し・要約・ラベル・節タイトルを出すことはプロトコル違反です（コード・API・ログ・パスは必要な言語のまま）。H2以上では実際に読んだパスと範囲をセッションのtask.mdへ記録します。H0は永続記録不要、H1は短い記録で足ります。履歴行の未検出は見直し候補であり、未読の証明ではありません。task.md / implementation_plan.md / walkthrough.md は現在タスク用のMarkdown証跡であり、ネイティブなタスク・プランUI状態ではありません。対応ランタイムでは Codex は update_plan、Claude Code は TaskCreate / TaskUpdate / TaskList / TaskGet も併用し、TodoWrite は古いランタイム向けのフォールバックに限定します。非自明なタスク（H2 以上）では axiarch-harness/ の Execution Harness が必須です。ロールパスを実行し、監査判定と証跡パケットを出し、axiarch-harness/{lang}/HUMAN_APPROVAL_GATE.mdの人間承認ゲートを適用し、必要な承認がない場合だけ停止してください。読み取り専用のサブエージェント委任はそれ自体では人間承認ゲートではありません。Deep Security Scan などの名前付き読み取り専用 workflow が明示された場合、必要な worker fanout はその要求に含まれます。最終判断はメインエージェントが保持します。教訓を作成・更新するときは axiarch-rules/{lang}/CRYSTALLIZATION_PROTOCOL.md のStep 1–6（件数・経過日、索引更新を含む）に従います。H0の読取だけに教訓整理は要求しません。
EOF

# v1.6.0+ short-circuit reminder (used after TTL window when no review hints)
read -r -d '' SHORT_REMINDER <<'EOF' || true
[AXIARCH REMINDER] axiarch governance applies to this project. Continue applying AXIARCH.md / axiarch-rules/{lang}/LOADING_PROTOCOL.md. Follow the latest explicit user language instruction, then respond in the Project Native Language declared in AXIARCH.md — every heading, summary, label, list, table, and bullet (code, APIs, logs, paths excepted); when it is Japanese, English headings/labels are a protocol violation. For non-trivial tasks (H2+), apply the Execution Harness (axiarch-harness/): role passes, audit verdict, evidence packet, and apply the human approval gate in axiarch-harness/{lang}/HUMAN_APPROVAL_GATE.md; pause only if required approval is missing. Read-only subagent delegation is not a human approval gate; explicit Deep Security Scan requests include required read-only fanout. Keep native task/plan state in sync when available. Full reminder reappears on TTL expiry, review hints, or new session. / axiarch ガバナンスは本プロジェクトに適用されます。AXIARCH.md / axiarch-rules/{lang}/LOADING_PROTOCOL.md の適用を継続。ユーザーの明示言語指定を最優先し、それ以外では応答は AXIARCH.md の Project Native Language で行い、すべての見出し・要約・ラベル・箇条書き・表に適用する（コード・API・ログ・パスは例外）。日本語の場合、英語の見出し・ラベルはプロトコル違反です。非自明なタスク（H2 以上）では axiarch-harness/ の Execution Harness（ロールパス・監査判定・証跡パケット・人間承認ゲート）を適用し、axiarch-harness/{lang}/HUMAN_APPROVAL_GATE.mdに従って必要な承認がない操作の前で停止してください。読み取り専用のサブエージェント委任は人間承認ゲートではありません。明示された Deep Security Scan は必要な読み取り専用 fanout を含みます。利用可能ならネイティブなタスク・プラン状態も同期してください。TTL 期限切れ・見直し候補・新規 session 時に full reminder が再表示されます。
EOF
