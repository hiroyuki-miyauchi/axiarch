...
| Language discipline | Project Native Language controls owner-facing plans, task evidence, specs, audits, walkthroughs, and approval requests unless the latest explicit user instruction says otherwise | Project Native Languageは、最新の明示的ユーザー指示が別途ある場合を除き、オーナー向け計画、タスク証跡、仕様、監査、walkthrough、承認依頼の言語を決める |
