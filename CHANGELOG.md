...
- Fix Claude Write borrowing Codex overwrite exceptions through an inherited agent hint. Prioritize native events and Claude settings, allowlists or explicit selection; invalid events reject existing-file replacement. Preserve the bounded legacy standalone Write fallback, new-file creation and each agent's approved paths, with bilingual migration guidance and isolated regressions.
...
- 日英の生成タスク・計画・結果テンプレートも同じ代替手順へ接続。古い自動退避先だけを過去記録の保存先として案内せず、sessions一覧と記録契約から確認する。新規生成の案内を修正し、再開時の既存文書は保持する。
- Connect generated Japanese/English task, plan and walkthrough templates to the same fallback. Locate previous work through the session listing and record contract instead of directing all history to the retired archive location. Update newly generated guidance while preserving documents on resume.
...
- **Programming Language Governance / プログラミング言語ガバナンス** — `axiarch-rules/{ja,en}/universal/engineering/320_programming_language_governance.md` を追加。TypeScript / JavaScript / HTML / CSS、Node.jsと条件付きDeno / Bun、Python、Java / Kotlin、C# / .NET、Go、Rust、PHP、Ruby、Lua、Perl、VBA、ABAP、Apex、Swift、Dart、SQL、R、Scala、Terraform / HCL、Bicep / cloud-native IaC、WebAssembly / WASI成果物、Dockerfile / Containerfile、Shell、PowerShell、YAML、Rego、Make / CMake、Bazel / Starlark、Nix、CUE / Jsonnet、C / C++、Solidity / Move、SystemVerilog / VHDL等に加え、主要web／backend frameworkとdesktop profile、GraphQL／graph query、observability query、dbt／DAX／Power Query M等のsemantic／transformation DSL、OpenTofu／Helm／Kustomize／Ansible等のinfra DSLを、支持区分、採用契約、evaluator／effective artifact、権限、lineage、cost、framework lifecycle、trust／release境界、言語ネイティブ品質gate、言語間contract、accountable ownerと継続経路、ポリグロットCI、SBOM / provenance、例外・移行・廃止まで一貫して統制する。Universal Rulesは50→51、Engineeringは14→15。
- **React Native Engineering / React Nativeエンジニアリング** — `axiarch-rules/{ja,en}/universal/engineering/420_react_native.md` を追加。React Nativeを言語ではなくTypeScript / JavaScript、Swift、Kotlinをまたぐframeworkとして明確化し、framework-managed / bare / brownfield / library profile、support window、New Architecture、Hermes、Codegen、Turbo Native Modules、Fabric、両OSのtest / release、security、OTA互換性・署名・段階配信・rollback、統合SBOM、symbolication、規模に応じたMobile Platform機能、upgrade運用、期限付き例外まで17セクション・Rule 420.1–420.52で統治する。Universal Rulesは51→52、Engineeringは15→16。
- **Cloud & Application Platform Governance / クラウド・アプリケーションプラットフォーム統治** — `axiarch-rules/{ja,en}/universal/engineering/520_cloud_application_platforms.md` を追加。Vercel、Supabase、Firebase/GCP、Cloudflareと主要cloud/PaaS/Kubernetes、AWS Amplify／Appwrite／Convex等のBaaSを、能力ベース選定、共有責任、team access、環境分離、artifactとmanaged runtime lifecycle、SDK support tier・feature parity・生成client・protocol fallback、preview/promotion/rollback、workload identity、data/migration/backup、security、observability/SLO、FinOps、quota、exit/DR、Platform Engineering、async event delivery、local／emulator conformance、provider-managed integration、multi-service aggregate release、BaaS capability manifest、trust surface、capability別lifecycle、identity portability、service EOL再検証、期限付き例外まで22セクション・Rule 520.1–520.89で統治する。Universal Rulesは52→53、Engineeringは16→17。
- **Microsoft Azure Cloud Governance / Microsoft Azureクラウドガバナンス** — `axiarch-rules/{ja,en}/universal/engineering/530_azure_cloud.md` を追加。Azure採用を前提化せず、tenant／management group／landing zone、subscription vending、Microsoft Entra／RBAC／PIM／managed identity、Azure Policy、Bicep等のIaC、App Service／Functions／Container Apps／AKS、言語・SDK lifecycle、Azure Functionsのmanaged language／Go Preview／Custom Handler／worker model／hosting plan、slot／revision、Private Link／Front Door、Key Vault、data／migration／backup、Service Bus／Event Grid、Azure Monitor、security／privacy、reliability／DR、Cost Management／quota、Platform Engineering、SBOM／provenance、managed conformance、exit／decommissionを21セクション・Rule 530.1–530.85で統治する。SKU、region、tool、固定人数・閾値はBlueprint parameterまたは時点付き参考実装へ分離する。Universal Rulesは53→54、Engineeringは17→18。
- **Universal Multi-Service Release Topology / 汎用multi-serviceリリース構成** — `axiarch-rules/{ja,en}/universal/engineering/100_api_integration.md` のservice分割、protocol、同期／非同期、Data Mesh、Saga／Outbox／CDC、consistency、progressive deliveryを成果ベースへ改定。`quality/000_qa_testing.md`のcontract test適用表とMicro Frontend releaseも同じ契約へ同期した。全service／frontend unitへの独立deploy・固定deploy順排除の強制を廃止し、independent／coordinated／aggregateのrelease topology、明示的互換window、順序gate、route／binding、partial failure、managed smoke、rollback／forward fixを統治する。
...
- **Universal applicability and stable rule identifiers / Universal適用条件・Rule識別子安定化** — 日英`engineering/100`のAPI Product、Contract-First、code generation、protocol、public client、token TTL、SDK、HTTP/3、Kubernetes Gateway API、reviewをconsumer、trust boundary、runtime、規制、運用能力に比例する契約へ変更。日英`engineering/510`、`operations/600`、`quality/000`、`security/000`、`security/200`のmanaged-service、ClickOps、専任CoE、Portal、Golden Path、team ranking、SBOM／CBOMの一律指定もscope付き成果契約へ変更した。新設正本のRule識別子は文書番号へ名前空間化し、`320.1–320.86`、`420.1–420.52`、`520.1–520.89`、`530.1–530.85`として既存API／AWS／AI規則との曖昧な衝突を回避し、health回帰検査を同期した。
- **Framework, DSL, desktop, notebook, and BaaS capability governance / Framework・DSL・desktop・notebook・BaaS能力統治** — 日英`engineering/320`を19セクション・Rule 320.1–320.86へ拡張し、framework／desktop、query／semantic／observability／template／infra DSL、公開library／SDK／packageのcompatibilityとdistributionに加えてnotebook／literate computational artifactのexecution、trust、production、team lifecycleを追加した。日英`engineering/520`を22セクション・Rule 520.1–520.89へ拡張し、BaaS capability manifest、trust surface、capability別lifecycle、identity portability、service EOL再検証をprovider中立な成果契約として追加した。
- **Canonical-rule tracking gate / 正本Git追跡ゲート** — health diagnosticのrelease-critical trackingへ日英`engineering/320`、`420`、`520`、`530`を追加し、新設正本が索引や公開文書から参照されながらuntrackedのままrelease候補へ漏れる状態をblockする。
- **Provider profile genericity / プロバイダープロファイル汎用化** — 日英`engineering/200`、`500`、`510`へUniversal適用契約を追加または強化。Supabase固定SSOT、Cloudflare/Vercel/Supabase固定stack、Firebaseの補助限定、Firestore新規利用禁止、全projectのBlaze固定、固定region・予算閾値・runtime・TypeScript・Terraformを唯一解にする規則を、能力評価、権威データ境界、risk-based gate、公式仕様再検証、Blueprint parameterへ置き換えた。App CheckはAuth、Security Rules、IAMの代替ではなく、budget alertはhard capではない境界も明記した。
- **Supabase and Firebase operational safety / Supabase・Firebase運用安全性** — 日英`engineering/200`の一律`SECURITY DEFINER`・全policyのadmin bypass・remote-first schema変更・固定plan／backup周期・GitHub Actions固定を、invoker既定、resource-scoped privileged access、migration-first、resource-complete recovery、実効role能力、provider-neutral CI/CDへ修正。日英`engineering/500`のApp Check／Rules／IAM／VPC一律構成、全queryの`.limit()`、固定10KB、特定relational backend優先、全function個別service account、全traffic VPC、emulator-onlyを、threat・capability・fidelity・bounded-read・short-lived identity契約へ置換した。
- **Current provider contract refresh / 現行provider契約の更新** — Supabaseのpublishable／secret key、Data API公開境界、RLS bypass、backup capability、Realtime・queue・pooler・vector検索を実効設定とrisk-based evidenceへ統合。Firebaseのretry／idempotency、`functions.config()`廃止移行、token revocation、App Check責任分離、Firebase Studioの新規workspace停止と2027年sunsetを公式資料スナップショットへ接続した。
- **Cross-cutting delivery, recovery, quality, and FinOps contracts / Delivery・復旧・品質・FinOps横断契約** — 日英`operations/400`のmanual deploy全面禁止、GitHub／Node固定gate、固定canary率、one-step rollback、固定RPO／RTO・restore周期・budget率を、追跡可能なdelivery、language-native gate、change-class recovery、risk-based Blueprint値へ修正。日英`quality/000`のTypeScript／npm専用release表、severity名だけのblock、固定rollout率・観測時間・synthetic周期をartifact／provenance／risk契約へ変更し、日英`operations/600`へprovider・専任組織・tool・price・割引率・閾値・cadenceを時点付きreferenceまたはBlueprint parameterとして扱うUniversal適用契約を追加した。
- **Accelerator, shader, and eBPF governance / アクセラレータ・shader・eBPF統治** — 日英`engineering/320`をCUDA C++、HIP C++、SYCL C++、Triton、OpenCL C、WGSL、GLSL、HLSL、Metal Shading Language、eBPFへ拡張。名称列挙だけでなく、host／device compiler・runtime・driver・backend matrix、CPU／referenceとの差分test、precision・NaN／Inf・determinism、memory・barrier・race、shader interface・binding・golden image、eBPF verifier・BTF・CO-RE・attach／detach、sourceからSPIR-V／DXIL／metallib／ELFまでのSBOM・provenanceを交換可能な成果契約として追加し、health回帰検知と公開導線を同期した。
- **Universal language applicability / 言語規約のUniversal汎用性** — 日英`engineering/320`、`410`、`420`へ、規範成果、ecosystem-native基線、非規範のreference implementation、risk-basedなBlueprint parameterを分離する適用契約を追加。固定人数、専任team、四半期／年次、GitHub系変更機能、Hilt、Pigeon、Firebase Test Lab、Xcode Cloud、固定coverage／build時間／rollout率を唯一の適合手段から外し、個人・小規模teamから規制対象企業まで同じ安全・再現性・所有・検証・退出成果で運用できる形へ改定した。
- **Enterprise, scientific, and managed-runtime coverage / 企業・科学技術計算・managed runtime対応** — 日英`engineering/320`へScala／Groovy／Clojure、Elixir／Erlang／Gleam、F#／OCaml／Haskell、COBOL／PL/I、Power Fx、Julia／MATLAB／Fortran／Mojo、SAS／Stataの条件付きtoolchain、長期保守、数値再現性、proprietary runtime退出契約を追加。Objective-C／Java mobileは一律書換えではなく段階移行・限定保守・support条件付き継続を選べる形へ修正した。日英`engineering/000`のDX章はDev Container、固定行数、固定人数、固定cadence／metric閾値をUniversal義務から外し、再現性、first-contribution SLO、knowledge continuity、privacy-aware measurementの成果契約へ変更。日英`engineering/200`へSupabase Edge Functionsのnested request-chain limit、日英`engineering/520`へCloudflare Workersの時点付きruntime maturityとenterprise／regional／sovereign cloud profileを追加し、README、INDEX、compliance、llms、health回帰検査を同期した。
- **Cross-cutting genericity hardening / 横断汎用性の強化** — 日英`engineering/000`、`quality/000`、`security/200`、`operations/400`へ同じUniversal適用契約を展開。固定20% capacity、重大欠陥の一律24時間修正完了、400変更行、browser最新2版、GitHub PR／CODEOWNERS、CTO等の役職、AGPL一律禁止、CVSS単独blockをUniversal義務から外し、即時triage／封じ込め、risk、法令・契約、official deadline、利用者分布、KEV／EPSS、reachability、配布形態に基づくBlueprint parameterとVCS-neutralな変更証跡へ置き換えた。既存codeを公式のversion別契約より常に優先する旧規則、固定10ファイル承認、特定BaaS唯一解、固定DB pool式／timeout、手動deploy全面禁止、特定provider専用IPv4復旧、credentialの固定90／180／365日、完全自動rotation唯一解も、導入版との照合、risk-based approval、hosting／data／edge／email／connectionの能力要件、監査可能なCDと統制されたbreak-glass、provider-neutralなnetwork診断、短命identity優先とprovider／risk／event基準へ変更した。privacy法令は法域、entity、処理、data subject、product scope、適用日から個別判定する。QAのDORA／CRA／SOC 2 mappingも適用対象、想定利用期間、法定trigger、assessment periodを確認する成果契約へ修正し、自動testだけで法的適合を断定しない境界を追加した。
- **License, package, and SBOM outcome contracts / ライセンス・パッケージ・SBOM成果契約** — 日英`security/200`のlink／SaaS license判断、lock、update bot、monorepo、registry、SCA、AI-assisted code、NIS2、Wasm、reproducible build、SBOM Federation／quality、ML-BOM、dependency SLOを再監査。GPL／AGPL等の一律allow／deny、pnpm・Snyk・Socket.dev・Dependency-Track・cosign・OCI・ORAS等の固定実装、法令・契約根拠のない14日・50%・70点・72時間等の固定値を、利用・配布形態、正確なlicense expression、法域、artifact digest、consumer contract、risk-based SLA、期限付き例外で判定する交換可能な成果契約へ変更した。CycloneDX公式predicate typeを`https://cyclonedx.org/bom`へ修正し、NIS2の移管状況と適用対象entityに対する法定24時間／72時間の報告境界を一次資料へ合わせ、health checkに固定断定の再発検知と日英Rule数parityを追加した。
- **Polyglot rule integration / ポリグロット規則統合** — `engineering/000` と `quality/000` の一律kebab-case、npm / TypeScript専用gate、lockfile削除復旧を言語中立契約へ修正。`security/000` のsecure coding差分と `security/200` のlockfile / SCA / SBOM matrixを主要backend・infra・systems言語とVBAへ拡張し、Dart／Flutterのapplicationと公開libraryで異なるlockfile境界、Terraform／OpenTofuのprovider lockとremote module固定、SLSA v1.2のBuild / Source Track、Source L4二者reviewとVSAの`SLSA_SOURCE_TWO_PARTY_REVIEWED`属性、uvのlocked CycloneDX直接出力、.NET 10 CLI、SREのlockfile復旧規律を日英同期。日英INDEX、README、準拠表、llms、health導線も同期し、`engineering/000` の古い13 / 80 / 145 / 160表記は実見出しと目次合計に合わせ22パート・141セクションへ統一。
- **React Native cross-cutting integration / React Native横断統合** — `engineering/320`と`410`の正本委譲、`quality/000`のJS／native／device多層gate、`security/000`のbundle secret・AsyncStorage・native boundary・deep link・OTA統制、日英INDEX／README／準拠表／llms／health回帰検出を同期。
- **Native language baseline refresh / ネイティブ言語基線更新** — `engineering/410`の日英を2026-07-23の公式安定版Kotlin 2.4.10 / Swift 6.3へ更新。固定版の追従だけでなく、Kotlin／AGP／D8／R8／JDK／Compose互換性、K1終了、Context Parameters安定化と旧preview flag除去、KSP段階移行、Kotlin/NativeのLLVM 21／CMS GC、Swift Export Alpha、Swift `@c`、Swift SDK for Android、Swift Build previewの採用境界を追加し、将来の版更新はsupport window、exact pin、ADR、互換性testで運用する。
- **Wasm / WASI factual baseline / Wasm・WASI事実基線** — 公式一次資料に合わせ、WebAssembly 3.0の完成日を2025-09-17、WASI 0.3.0の公開日を2026-06-11として日英`engineering/000`、SRE、言語ガバナンスの参照、health回帰検査を整合。core spec、Component Model、WASI、各engineの対応を別の互換性判断として扱う境界は維持する。
- **Team Git and worktree safety / チームGit・worktree安全性** — `engineering/600`の日英へUniversal適用契約を追加し、特定projectの障害履歴を匿名のfailure patternへ変更。Git公式仕様に従い`extensions.worktreeConfig`を正式機能として保持し、`check-git-config-clean.sh`はGitがprune可能と判定する管理情報と、実在しないlocal branchのconfigだけをdry-run後に修復する。`--full-clean`によるbranch強制削除を廃止し、active worktree、branch ref、未保存変更を保護するfixtureを追加検証した。
