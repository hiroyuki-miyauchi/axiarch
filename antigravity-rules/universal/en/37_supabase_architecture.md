...
## Appendix A. Service Quick Reference Index

> **Purpose**: A reverse lookup index for quickly locating rules for target services across 40 sections and 138 rules.

| Supabase Service | Related Sections |
|:----------------|:----------------|
| **PostgreSQL / Database** | §2, §4, §9, §31, §36, §38, §40 |
| **RLS (Row Level Security)** | §3, §5.1, §8.3, §11.3, §12.3, §12.6, §19.1, §29.1, §38.1 |
| **Auth (GoTrue)** | §5, §5.2, §12.1, §18, §25.1, §33, §35, §39 |
| **Storage** | §6, §6.1, §6.2, §2.11 |
| **Edge Functions** | §13, §19.2, §25.2 |
| **Realtime** | §14, §37.2 |
| **pg_cron / Cron** | §15.1 |
| **Queues (pgmq)** | §15.2 |
| **Database Webhooks** | §15.3 |
| **Migrations** | §7, §11.5, §11.9, §12.2, §40 |
| **Type Safety** | §2.3, §11.4, §12.4 |
| **FinOps / Cost** | §16.3 |
| **Observability / Monitoring** | §16.1, §16.2, §16.4, §32, §36.2 |
| **pgvector / AI** | §17 |
| **MFA / PKCE** | §18.2 |
| **Testing** | §19 |
| **Branching / Environments** | §20 |
| **PostgREST / REST API** | §21, §28, §30.2 |
| **CLI / Local Dev** | §22, §40.2 |
| **Connection Pooling / Supavisor** | §23 |
| **Backup / DR** | §24 |
| **Rate Limiting / API Protection** | §25 |
| **Vault / Secret Management** | §26 |
| **Foreign Data Wrappers** | §27 |
| **Data API Hardening** | §28 |
| **Multi-tenancy** | §29 |
| **pg_graphql / GraphQL** | §30 |
| **DB Functions / Triggers** | §31 |
| **Log Drain / External Observability** | §32 |
| **Auth Hooks / Custom Claims** | §33 |
| **Self-hosted / Email** | §34 |
| **SSR / Framework Integration** | §35 |
| **Database Extensions** | §36 |
| **Client SDK / supabase-js** | §37 |
| **Schema Design Patterns** | §38 |
| **Social Auth / OAuth / SSO** | §39 |
| **Data Migration / Seeding** | §40 |

### Internal Cross-References
-   **Idempotent Migrations**: §7.4 (Foundation) → §11.5 / §12.2 (Advanced)
-   **RLS InitPlan Optimization**: §3.0 (Law 4: Scalar Subquery) → §12.3.1 / §12.6 (Detailed Implementation)
-   **Data Archival**: §2.14 (Cold Data Offloading) → §2.13 (Time-Series Partitioning) → §2.18 (Retention Protocol)
-   **Admin Privileges**: §11.6 / §12.1 (Service Role) → §3.0.2 (Admin RLS Mandate) → §12.7 (Client Identity Audit)
-   **Auth Security**: §5 (Auth Basics) → §5.1 (RLS by Default) → §5.2 (Session Mgmt) → §18.1 (API Keys) → §18.2 (PKCE/MFA) → §18.3 (Anonymous Auth) → §25.1 (Rate Limits) → §33 (Auth Hooks) → §35 (SSR Integration) → §39 (Social Auth)
-   **Storage**: §6 (Basics) → §6.1 (S3 Compatible) → §6.2 (Image Transformations) → §2.11 (Orphan File Defense)
-   **Testing Strategy**: §19.1 (RLS Testing) → §19.2 (Edge Functions Testing) → §19.3 (DB Function Testing)
-   **Environment Management**: §20.1 (Environment Isolation) → §20.2 (Branching) → §22 (CLI) → §7 (Migration) → §34 (Self-hosted) → §40.2 (Seeding)
-   **API Design**: §21.1 (Select Optimization) → §21.2 (Embedding) → §21.3 (Pagination) → §2.3 (Type Safety) → §30 (GraphQL)
-   **Connection Management**: §23.1 (Supavisor) → §23.2 (Pool Size) → §23.3 (Connection Mode) → §4 (Performance)
-   **Data Protection**: §24.1 (Backup) → §24.2 (DR Planning) → §2.18 (Data Retention) → §16.3 (FinOps)
-   **API Protection**: §25.1 (Auth Rate Limits) → §25.2 (Custom Rate Limiting) → §13 (Edge Functions)
-   **Secret Management**: §26.1 (Vault Encryption) → §26.2 (Rotation) → §18.1 (API Key Mgmt)
-   **External Data Integration**: §27.1 (FDW Architecture) → §27.2 (Security & Perf) → §26.1 (Vault)
-   **API Attack Surface Reduction**: §28.1 (Schema Exposure) → §28.2 (Network Lockdown) → §21 (PostgREST) → §3 (RLS)
-   **Tenant Isolation**: §29.1 (tenant_id Design) → §29.2 (Isolation Audit) → §3 (RLS) → §19 (Testing)
-   **GraphQL**: §30.1 (pg_graphql Design) → §30.2 (REST vs GraphQL) → §21 (PostgREST) → §28 (API Hardening)
-   **Server-Side Logic**: §31.1 (Function Security) → §31.2 (Trigger Design) → §3 (RLS) → §28 (API Hardening)
-   **External Observability**: §32.1 (Log Drain) → §32.2 (Metrics & Alerting) → §16 (Internal Observability)
-   **Auth Customization**: §33.1 (Custom Access Token Hook) → §33.2 (Auth Trigger) → §31 (DB Functions) → §29 (Multi-tenancy)
-   **Self-hosted Operations**: §34.1 (Docker Compose) → §34.2 (SMTP/Email) → §26 (Vault) → §24 (Backup)
-   **SSR Integration**: §35.1 (@supabase/ssr Design) → §35.2 (Middleware Auth) → §5 (Auth) → §33 (Auth Hooks)
-   **Extension Management**: §36.1 (Extension Governance) → §36.2 (pg_stat_statements) → §4 (Performance) → §16 (Observability)
-   **Client SDK**: §37.1 (Error Handling) → §37.2 (Realtime Lifecycle) → §14 (Realtime) → §35 (SSR)
-   **Schema Design**: §38.1 (Soft Delete) → §38.2 (Audit Trail/JSONB) → §2 (DB Design) → §31 (Triggers)
-   **Social Auth**: §39.1 (OAuth Providers) → §39.2 (SAML SSO/Deep Linking) → §5 (Auth) → §18 (Advanced Auth)
-   **Data Migration**: §40.1 (pg_dump/pg_restore) → §40.2 (Seed Management) → §7 (Migration) → §22 (CLI)
