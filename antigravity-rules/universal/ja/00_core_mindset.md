...
*   **Law**: **AIはいかなる理由があっても、ユーザーからの明示的な指示（「コミットして」「保存して」等）がない限り、Gitコマンド（add, commit, push, merge, rebase等）を実行してはなりません。**
*   **Action**:
    1.  **Wait**: 作業完了後は、ファイルを保存した状態で待機し、`git status` で変更内容を提示するに留めます。
    2.  **Ask**: 「コミットしてもよろしいですか？」とユーザーに伺いを立て、承認を得てからのみコマンドを実行します。
    3.  **No Assumption**: 「キリが良いから」という自己判断は一切禁止です。

### 8.2. The Main Branch Sanctuary
*   **Law**: `main` (または `master`) ブランチへの直接のコミットおよび作業は、物理的に禁止とします。
*   **Action**:
    *   `git branch` が `main` を示している場合、いかなるコード編集も開始してはなりません。
    *   必ず適切な命名のブランチ（`feature/xxx`, `fix/xxx`）を新規作成し、移動してから着手してください。
