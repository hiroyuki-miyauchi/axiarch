...
## Appendix A. サービス別逆引き索引 (Service Quick Reference Index)

> **目的**: 40セクション・138ルールの中から対象サービスのルールを即時発見するための逆引き索引。
...
### 内部クロスリファレンス
-   **冪等マイグレーション**: §7.4（基礎）→ §11.5 / §12.2（発展）
-   **RLS InitPlan最適化**: §3.0（Law 4: Scalar Subquery）→ §12.3.1 / §12.6（詳細実装）
-   **データアーカイブ**: §2.14（Cold Data Offloading）→ §2.13（Time-Series Partitioning）→ §2.18（Retention Protocol）
-   **管理者権限**: §11.6 / §12.1（Service Role）→ §3.0.2（Admin RLS Mandate）→ §12.7（Client Identity Audit）
-   **認証セキュリティ**: §5（Auth基礎）→ §5.1（RLS by Default）→ §5.2（Session管理）→ §18.1（APIキー）→ §18.2（PKCE/MFA）→ §18.3（Anonymous Auth）→ §25.1（Rate Limits）→ §33（Auth Hooks）→ §35（SSR統合）→ §39（Social Auth）
-   **ストレージ**: §6（基礎）→ §6.1（S3互換）→ §6.2（Image Transformations）→ §2.11（孤立ファイル防衛）
-   **テスト戦略**: §19.1（RLSテスト）→ §19.2（Edge Functionsテスト）→ §19.3（DB関数テスト）
-   **環境管理**: §20.1（環境分離）→ §20.2（Branching）→ §22（CLI）→ §7（Migration）→ §34（Self-hosted）→ §40.2（Seeding）
-   **API設計**: §21.1（Select最適化）→ §21.2（Embedding）→ §21.3（ページネーション）→ §2.3（Type Safety）→ §30（GraphQL）
-   **接続管理**: §23.1（Supavisor）→ §23.2（プールサイズ）→ §23.3（接続モード）→ §4（Performance）
-   **データ保護**: §24.1（バックアップ）→ §24.2（DR計画）→ §2.18（データ保持）→ §16.3（FinOps）
-   **API保護**: §25.1（Auth Rate Limits）→ §25.2（カスタムRate Limiting）→ §13（Edge Functions）
-   **シークレット管理**: §26.1（Vault暗号化）→ §26.2（ローテーション）→ §18.1（APIキー管理）
-   **外部データ統合**: §27.1（FDWアーキテクチャ）→ §27.2（セキュリティ&性能）→ §26.1（Vault）
-   **API攻撃面縮小**: §28.1（スキーマ公開制限）→ §28.2（Network遮断）→ §21（PostgREST）→ §3（RLS）
-   **テナント分離**: §29.1（tenant_id設計）→ §29.2（分離監査）→ §3（RLS）→ §19（テスト）
-   **GraphQL**: §30.1（pg_graphql設計）→ §30.2（REST vs GraphQL）→ §21（PostgREST）→ §28（API Hardening）
-   **サーバーサイドロジック**: §31.1（関数セキュリティ）→ §31.2（トリガー設計）→ §3（RLS）→ §28（API Hardening）
-   **外部Observability**: §32.1（Log Drain）→ §32.2（Metrics & Alerting）→ §16（内部Observability）
-   **認証カスタマイズ**: §33.1（Custom Access Token Hook）→ §33.2（Auth Trigger）→ §31（DB Functions）→ §29（Multi-tenancy）
-   **セルフホスト運用**: §34.1（Docker Compose）→ §34.2（SMTP/Email）→ §26（Vault）→ §24（Backup）
-   **SSR統合**: §35.1（@supabase/ssr設計）→ §35.2（Middleware認証）→ §5（Auth）→ §33（Auth Hooks）
-   **拡張管理**: §36.1（Extension Governance）→ §36.2（pg_stat_statements）→ §4（Performance）→ §16（Observability）
-   **Client SDK**: §37.1（エラーハンドリング）→ §37.2（Realtime Lifecycle）→ §14（Realtime）→ §35（SSR）
-   **スキーマ設計**: §38.1（Soft Delete）→ §38.2（Audit Trail/JSONB）→ §2（DB設計）→ §31（Triggers）
-   **Social Auth**: §39.1（OAuthプロバイダ）→ §39.2（SAML SSO/Deep Linking）→ §5（Auth）→ §18（Advanced Auth）
-   **データ移行**: §40.1（pg_dump/pg_restore）→ §40.2（Seed管理）→ §7（Migration）→ §22（CLI）
