...
*   **生体認証 (Biometrics)**:
    *   **必須**: 決済や機密情報の閲覧には、必ず **FaceID / TouchID** (iOS) および **BiometricPrompt** (Android) を実装します。
*   **ハプティクス (Haptics)**:
    *   **フィードバック**: 成功、失敗、警告などの重要なアクションには、プラットフォーム標準のハプティクスフィードバック（`UIImpactFeedbackGenerator` 等）を使用します。

## 5. Flutter Integration (Platform Channels)
### 5.1. Type Safety (Pigeon)
