...
*   **Cross-Ref**: ユーザーサポートメールは`51_user_support.md`参照。
...
*   **Cross-Ref**: AI実装全般は`40_ai_implementation.md`参照。Connectは§84参照。
...
> **目的**: 141セクション・210ルール以上の中から対象サービスのルールを即時発見するための逆引き索引。
...
- **ネットワークハブ**: §130（Transit Gateway）→ §131（Direct Connect）→ §2（VPC）→ §92（PrivateLink）
- **セキュリティ調査チェーン**: §6（GuardDuty検知）→ §132（Detective調査）→ §111（インシデント対応）→ §126（CloudTrail Lake監査）→ §138（DevOps Agent AI支援）

### クロスリファレンス（他ルールファイル）
- **Supabase Backend**: `37_supabase_architecture.md` — BaaS戦略、RLS、Edge Functions
- **Security & Privacy**: `60_security_privacy.md` — ゼロトラスト、OWASP Top 10
- **SRE & Reliability**: `52_sre_reliability.md` — 可用性99.99%、カオスエンジニアリング、負荷テスト
- **API Integration**: `35_api_integration.md` — REST/GraphQL設計、Rate Limiting
- **Engineering General**: `30_engineering_general.md` — コードレビュー、テスト戦略
- **AI Implementation**: `40_ai_implementation.md` — ストリーミングファースト、RAG設計、トークンコスト管理
