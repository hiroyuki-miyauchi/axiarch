...
- [00. Project Overview & Constitution](00_project_overview.md) - プロジェクトビジョン・技術スタック・不変の原則。
- [01. Project Lessons Log](01_project_lessons_log.md) - 開発を通じて得られた「教訓」と「アンチパターン」の蓄積。
...
- [99. Project Specific Template](99_project_specific_template.md) - ルールファイル追加時のテンプレート。
...
3. `00_project_overview.md` をプロジェクトの内容に書き換える

### 新規仕様書の追加
1. `99_project_specific_template.md` をコピーする
2. 上記のカテゴリ番号帯に従ってファイル名を変更する（例: `60_feature_payment.md`）
3. テンプレートの各セクションをプロジェクト固有の内容で埋める
4. **このファイル（INDEX.md）の該当カテゴリにエントリを追記する**

### 教訓の記録
- 作業完了時や重要な決定時に、`01_project_lessons_log.md` に所定フォーマットで追記する
